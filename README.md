...
浏览网页时**选中文字 → 旁边自动浮出 webbuddy 图标 → 鼠标移上去选个预设问题 → 右侧侧边栏流式输出答案**，全程无需手动输入，不打断阅读；也可在侧边栏底部输入框连续追问。同时提供**图片/视频一键隐藏或缩略**、**免广告**、**解读当前页面**、**截图问图**等一站式能力。
...
选中任意网页上的文字，选区旁会**自动浮出一个 webbuddy 图标**（和工具栏图标同款：蓝底白问号）：

- **鼠标移上去**（约 0.1s）→ 展开预设问题清单，顶部显示选中的文字片段，下面就是设置页里那几个预设问题（Summarize in 3 lines / Find an analogy / …），最后一项是 **Custom question…**
- **点一下** → 直接打开侧边栏并开始回答，不用再走右键菜单
- 鼠标移开只收起清单、**图标留在原地**，方便再悬停；点页面其它地方 / 滚动 / 按 Esc 才清掉
- 输入框、`textarea`、可编辑区域里的选中**不打扰**（那属于编辑操作）

设计要点：

- 气泡挂在 **Shadow DOM** 里，字体/按钮样式/z-index 都不受宿主页面影响；反过来，页面的 document 级监听器也收不到气泡内部的点击，所以**点击图标时选区不会消失**（在 `mousedown` 捕获阶段 `preventDefault` 保住选区）。
- 图标是内联 SVG，不占用 `web_accessible_resources`，页面上探测不到扩展资源地址。
- 只读配置：预设问题与开关都读自 `settings`，在设置页改完**页面即时生效**（无需刷新）。
- 提问前不会发送任何内容；**只有点了某一条**，才会把选中的文字交给当前标签页对应的侧边栏会话。
- 个别站点觉得碍事可以在设置页 → **Selection bubble** 关掉；**右键菜单「Ask AI」完整保留**，作为功能更齐全的入口。
...
1. **最快路径**：在任意网页选中文字 → 选区旁浮出 webbuddy 图标 → 鼠标移上去 → 点一条预设问题，直接出答案（见上节「划词气泡」）
2. 也可以走右键：选中文字 → 右键 → 「问 AI」→ 选一条预设（默认四条：三句话说清 / 类比定位 / 为什么重要 / 外行版解释）
3. **「📄 解读当前页面」**：右键任意位置（无需选中文字）→ 问 AI ▸ 解读当前页面，AI 会先抓取当前页面正文，再输出总体概览与可深入方向
4. 侧边栏自动打开并流式输出；生成中可「停止」，完成后可「复制」，失败可「重试」
5. 「✏️ 自定义问题…」（右键菜单与气泡里都有）会聚焦输入框，首次发送时自动附带选中文本与页面上下文
6. 底部输入框可直接追问；无选区时，若通过工具栏图标打开过面板，自由提问会自动携带当前页面上下文（AI 能感知你在看哪一页）
7. **截图问图**：点输入框左侧「截」→ 在页面上拖拽框选区域（Esc 取消）→ 截图回到侧边栏，弹出几条默认问图问题（可点选或自行输入）→ 发送即用视觉模型回答
8. 「＋」新建会话（清空时间线）；历史仅保留在当前浏览器会话内，关闭浏览器自动清空

预设问题的 label 同时用于**右键菜单**与**划词气泡**（气泡只显示 label，模板由后台按 id 取当前配置，所以改了模板不用刷新页面）。
...
```
入口 A（右键菜单 background.js）  ┐
入口 B（划词气泡 content/selection.js → SW askFromSelection）┘
  → chrome.storage.session 写任务 → 侧边栏 onChanged/兜底读取
...
content/            图片控制 content script（三态 class 切换）+ 划词气泡 selection.js + CSS 规则
...
- `host_permissions: <all_urls>`：图片控制与划词气泡需声明式注入所有页面才能「打开即生效」。安装/更新时浏览器会提示「读取所有网站数据」，此为本功能必需，不用于收集任何数据。

## 已知限制（v1.5）

- 页面正文抓取对超长页面只取节选（约 8000 字）；`chrome://`、Web Store 等受限页无法注入，退回仅标题+URL
- **Chrome 保留域（`chrome.google.com`、Chrome Web Store、`chrome://` 页面、默认新标签页）禁止任何扩展注入 content script**——这是浏览器级安全边界，声明再多权限也无法覆盖。在这些页面上划词气泡、图片控制、免广告均不生效（沙拉查词等所有划词类扩展同样如此）；**右键菜单问 AI 不受影响**（选区文字由浏览器原生提供，不依赖注入）
- 划词气泡：少数用 `transform` / `filter` 处理 `<html>` 的站点可能让气泡定位偏移（视口坐标被重新锚定）；iframe 内的选区正常支持；扩展刚重载后未刷新的页面需刷新一次才能用上新版气泡
...
- 本地自测（`tools/` 下，无需浏览器）：
  ...
  ```bash
  node tools/test_background_ask.js                      # 后台任务派发（vm 真加载 SW）
  NODE_PATH=<node工作区>/node_modules node \
    tools/test_selection_bubble.js                       # 划词气泡交互（jsdom，需 jsdom）
  ```

## 隐私

**没有服务器、没有账号、没有遥测。** 唯一离开浏览器的数据，是你主动提问时发出的那段文字/截图，
且直接发往**你自己配置**的 API 服务商（填本机 Ollama 则完全不出机器）。
API Key 只存本机 `chrome.storage.local`；问答历史存 `chrome.storage.session`，关浏览器即清。

完整的权限逐项解释、数据流向表、删除方式见 **[PRIVACY.md](./PRIVACY.md)**。

## 开源许可

[MIT License](./LICENSE) © 2026 luxviat — 可自由使用、修改、二次分发与商用，保留版权声明即可。

Tab Dock 为本项目自研的标签收纳实现，**不含、不依赖、不引用** OneTab 的任何代码或资源；
图标为本项目自行生成（`tools/gen_icons.py`）。
