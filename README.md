...
复刻 OneTab 的标签收纳能力，**完全本地、无服务器、可替代独立插件**。

- **侧栏入口**（顶栏最左的堆叠图标）：点「收入当前窗口」把当前窗口所有网页标签收成一组并**关闭它们释放内存**；列表里每个分组可「恢复全部 / 展开 / 回收站」。
- **管理页**（侧栏点「管理页 ↗」，或右键页面 → 🗂 OneTab → 打开 OneTab 管理页）＝全宽版 OneTab：
  - 左栏分类：**全部 / 星标 / 待办 / 已归档 / 回收站**（带计数），顶部搜索 + 「仅显示命名项」
  - 每张卡片 = 一次收纳 = 一组：标题（`N tabs` 或自定义名）、时间与相对时间、折叠、**恢复全部**、**⋮ 更多…**
  - **⋮ 更多…**：恢复到新窗口 / 本窗口 / 无痕窗口、复制标题与链接、重命名 / 加备注、锁定（恢复后保留本组）、星标、待办、归档、置顶、粘贴链接、上下新建分组、移除本组重复项、上移 / 下移、移到回收站
  - **拖拽归类**：卡片里的每个链接都能拖动——拖到别的卡片即换个分组，拖到组内其它位置即重排；拖空的组自动消失
- **一次性收纳一批**：网页上右键 → 🗂 OneTab → 收入当前标签 / 收入所有标签（全部窗口），同样会关闭标签释放内存。
- **导入 / 导出**：导出 JSON（完整备份）/ 纯文本（兼容 OneTab 导出格式），导入支持 JSON、OneTab 文本、或直接粘贴一堆链接。
- **与官方 OneTab 的差异**：① 去掉了「Share as a web page」（唯一需要上传服务器的功能，隐私取舍），纯本地存储；② 收纳时**不会关掉某个窗口的最后一个标签**，避免整窗（含侧边栏）一起消失；③ 数据在 `chrome.storage.local`，与侧栏/右键菜单实时同步。
...
onetab-store.js     OneTab 数据层（分组 / 回收站 / 恢复 / 去重 / 导入导出），页面·侧栏·SW 三端共用
onetab.html/.css    内置 OneTab 管理页（全宽独立页，1:1 复刻官方布局）
onetab-page.js      管理页交互：分类计数、卡片、⋮ 更多菜单、拖拽归类
background.js       Service Worker：右键菜单 + 任务分发 + 页面上下文捕获 + OneTab 收纳
content/            图片控制 content script（三态 class 切换）+ CSS 规则
sidepanel/          侧边栏（会话、流式渲染、追问 + 图片三段开关 + OneTab 收纳入口）
...
- `tabs`：内置 OneTab 需要读取所有标签的标题/网址才能收纳，并向 `chrome.tabs.create/remove` 恢复或关闭标签；仅本地使用，不联网。
...
- OneTab：`chrome://`、扩展页、about: 等非 http(s)/file 页面无法收纳；「恢复到无痕窗口」需在扩展详情里开启「允许在无痕模式下使用」；不提供官方那个「分享为网页」（需上传服务器）；收纳时不会关闭某窗口的最后一个标签
